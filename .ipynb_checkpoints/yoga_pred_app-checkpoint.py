...
st.set_page_config(page_title="Yoga Asana Prediction", page_icon=":lotus_position:", layout="centered", initial_sidebar_state="auto", menu_items=None)
...
st.markdown('### Upload an image with your Yoga Asana here:')
#file_uploaded = Image.open('data/test/downdog/00000159.jpg')
file_uploaded = st.file_uploader('', type = ['jpg','jpeg','png'])
if file_uploaded is not None:
    image = Image.open(file_uploaded)
else:
    st.warning('Please upload an Image from which you want to predict the yoga asana')
    
if st.button('Predict Yoga Asana'):
    figure = plt.figure()
    plt.imshow(image)
    plt.axis('off')
...
if st.button('<style>background-color: #eeeeee</style> Plot the Confusion Matrix'):
